...
const T = { th: 'px-3 py-2 text-xs font-medium text-gray-500 uppercase', td: 'px-3 py-1.5 text-sm' }

function Tbl({ title, head, children }) {
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
      <div className="px-3 py-2 font-semibold border-b">{title}</div>
      <table className="min-w-full divide-y divide-gray-200 text-xs">
        <thead><tr>{head.map((h) => <th key={h} className={T.th}>{h}</th>)}</tr></thead>
  ...
  return (
    <div className="space-y-5">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Fiber Capacity</h1>
          <p className="text-sm text-gray-500">OLT, PON-port and ONT utilization plus route and splitter inventory.</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
        {[
          ...
          <div key={l} className="bg-white rounded-lg border border-gray-200 p-3 text-center">
      ...
      {data.olts?.length > 0 && (
        <Tbl title="OLTs" head={['OLT', 'Status', 'Ports', 'Active', 'Util %', 'Max ONTs', 'Registered']}>
          {data.olts.map((o) => (
            <tr key={o.id}>
              <td className={T.td}>{o.name}</td>
              <td className={T.td}><Badge label={o.status} variant={SV[o.status] || 'inactive'} /></td>
              <td className={T.td}>{o.total_pon_ports}</td>
              <td className={T.td}>{o.active_pon_ports}</td>
              <td className={T.td}>{o.port_utilization_pct}%</td>
              <td className={T.td}>{o.max_ont_capacity}</td>
              <td className={T.td}>{o.ont_count}</td>
            </tr>
          ))}
        </Tbl>
      )}
        ...
        <Tbl title="PON Ports" head={['Port', 'OLT', 'Max ONTs', 'Registered', 'Util %']}>
          {data.pon_ports.map((p) => (
            <tr key={p.id}>
              ...
              <td className={T.td}>{p.olt}</td>
              <td className={T.td}>{p.max_onts}</td>
              <td className={T.td}>{p.registered_onts}</td>
              <td className={T.td}>{p.utilization_pct}%<Bar v={p.registered_onts} m={p.max_onts} /></td>
            </tr>
          ))}
        </Tbl>
      )}
      ...
      {data.routes?.length > 0 && (
        <Tbl title="Fiber Routes" head={['Route', 'Source → Destination', 'km', 'Type', 'Connections']}>
          {data.routes.map((r) => (
            <tr key={r.id}>
              <td className={T.td}>{r.name}</td>
              <td className={T.td}>{r.source} → {r.destination}</td>
              <td className={T.td}>{r.length_km}</td>
              <td className={T.td}>{r.cable_type}</td>
              <td className={T.td}>{r.connection_count}</td>
            </tr>
          ))}
        </Tbl>
      )}
      ...
      {data.splitters?.length > 0 && (
        <Tbl title="Splitters" head={['Splitter', 'Ratio', 'Location', 'Status']}>
          {data.splitters.map((sp) => (
            <tr key={sp.id}>
              <td className={T.td}>{sp.name}</td>
              <td className={T.td}>{sp.split_ratio}</td>
              <td className={T.td}>{sp.location}</td>
