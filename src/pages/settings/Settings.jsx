...
import { Eye, EyeOff, Building2, Receipt, Smartphone, MessageSquare, Settings as SettingsIcon } from 'lucide-react'
...
// field-level metadata: label, span (full width vs half), helper text, sensitivity
const FIELD_META = {
  ...
  invoice_prefix:        { label: 'Invoice Prefix',    span: 'half', helper: 'e.g. INV — prepended to invoice numbers' },
  tax_rate:              { label: 'Tax Rate (%)',      span: 'half' },
  grace_period:          { label: 'Grace Period (days)', span: 'half', helper: 'Days before overdue accounts are suspended' },
  ...
  sms_gateway:            { label: 'SMS Gateway',       span: 'half' },
  sms_sender_id:          { label: 'Sender ID',         span: 'half', helper: 'Max 11 characters, shown to recipients' },
  sms_api_key:            { label: 'API Key',           span: 'full', sensitive: true },

  timezone:               { label: 'Timezone',          span: 'half' },
    ...
    <div className={meta.span === 'full' ? 'col-span-2' : 'col-span-2 sm:col-span-1'}>
      ...
      <div className="relative">
        <input
          ...
          className="input"
          type={isSensitive && !reveal ? 'password' : 'text'}
      ...
      {meta.helper && (
        <p className="text-xs mt-1" style={{ color: 'var(--pb-text-3)' }}>{meta.helper}</p>
      )}
    </div>
  )
}
          ...
          {TABS.map(tab => {
            ...
            return (
              <button
                ...
                style={isActive ? {
                  background: 'linear-gradient(90deg, rgba(37,99,235,0.16) 0%, rgba(37,99,235,0.03) 100%)',
      ...
      <div className="flex-1 card">
        <div className="mb-6 pb-4" style={{ borderBottom: '1px solid var(--pb-border)' }}>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--pb-text-2)' }}>
            {activeTab === 'mpesa' || activeTab === 'sms'
              ? 'Sensitive credentials — click the eye icon to reveal a value before editing.'
        ...
        <div className="flex justify-end mt-8 pt-5" style={{ borderTop: '1px solid var(--pb-border)' }}>
          <button
